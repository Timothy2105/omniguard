...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'OpenAI API key not properly configured' }, { status: 500 });
  }
  try {
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are a helpful assistant monitoring a real-time video stream. You have access to detected events and can provide guidance, especially during dangerous situations. Be concise but informative, and show appropriate concern for dangerous events while remaining calm and helpful.',
        },
        {
          role: 'system',
