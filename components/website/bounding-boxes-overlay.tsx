...
import type { BoundingBoxData } from '@/app/types/index';
...
export function BoundingBoxesOverlay({ videoName, currentTime, fps = 30, width, height }: BoundingBoxesOverlayProps) {
  ...
  return (
    <div className="absolute inset-0 pointer-events-none">
      {currentBoxes.map((box, index) => {
        ...
        return <div key={index} className="absolute border-2 border-green-400 bg-green-400/20" style={scaledStyle} />;
      })}
    </div>
  );
}
